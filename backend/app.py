...
import re
import tempfile
import logging # For logging errors on the server
from flask import Flask, request, jsonify, send_from_directory
from flask_cors import CORS
import yt_dlp

# --- Configuration ---
# Set up basic logging to see errors in Render's Log Stream
...
def prepare_cookie_file():
    ...
    # Create a temporary, writable file for yt-dlp to use
    ...
    if source_path:
        with open(source_path, 'r') as secret_file:
            ...
            if not cookie_content.strip():
                ...
                return None # Return None if cookies are empty
            temp_cookie_file.write(cookie_content)
    else:
        logging.error("CRITICAL: No cookie file found.")
        return None # Return None if no cookie file exists
...
def get_info():
    ...
    try:
        ...
        # Log the real error for you to see in Render's logs
