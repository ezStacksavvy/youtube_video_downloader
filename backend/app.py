...
def get_info():
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # --- FIX: Ensure we always send the video_id ---
            return jsonify({
                'title': info.get('title', 'No title'),
                'thumbnail': info.get('thumbnail', ''),
                'video_id': info.get('id'), # This is the crucial part
...
def process_download():
    # --- FIX: We now accept video_id instead of url and title ---
    data = request.get_json()
    video_id = data.get('video_id')
    quality = data.get('quality')
    
    if not all([video_id, quality]):
        return jsonify({"error": "Missing video_id or quality parameter for download."}), 400
    
    try:
        # Reconstruct the URL on the backend, which is safer
        url = f"https://www.youtube.com/watch?v={video_id}"
        
        # We must re-fetch the title to create a safe filename
