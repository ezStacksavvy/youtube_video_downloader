...
from flask_cors import CORS, cross_origin # <-- IMPORTANT: Import cross_origin
import yt_dlp
...
app = Flask(__name__)
# This base CORS is still good as a fallback
CORS(app)
...
@cross_origin() # <-- THE FIX: Add this decorator
def get_info():
...
@cross_origin() # <-- THE FIX: Add this decorator here too
def process_download():
    ...
    try:
        ...
        ydl_opts = {'format': f'bestvideo[height<={height}][ext=mp4]+bestaudio[ext=m4a]/best[height<={height}][ext=mp4]','outtmpl': output_path,'quiet': True,'no_warnings': True, 'cookiefile': COOKIE_FILE_PATH}
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ydl.download([url])
